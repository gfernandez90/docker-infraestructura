...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ejecutar_sync'])) {
    try {
        ...
        // INSERT actualizado con parent_id
        $stmtIssue = $pdo->prepare("
            INSERT INTO redmine_tareas (
                id, proyecto_id, parent_id, tracker_nombre, estado_id, prioridad_id, asunto, descripcion,
        ...
        foreach ($proyectosObjetivo as $projIdentifier) {
            ...
            foreach ($issues as $issue) {
                ...
                // Insertar Tarea (incluye la resolución del parent_id)
                $stmtIssue->execute([
                    ':id'              => $issue['id'],
                    ':proyecto_id'     => $projData['id'],
                    ':parent_id'       => $issue['parent']['id'] ?? null, // <--- AQUÍ SE ASIGNA EL PARENT ID
                ...
                if (isset($issue['watchers']) && is_array($issue['watchers'])) {
                    foreach ($issue['watchers'] as $watcher) {
                        // Guardar usuario seguidor en la tabla de usuarios
                        $stmtUser->execute([
                            ':id'     => $watcher['id'],
                            ':nombre' => $watcher['name']
                        ]);
        ...
        $mensaje = "Sincronización completada exitosamente. Se importaron {$totalProyectos} proyectos y {$totalTareas} tareas con sus relaciones de parentesco y seguidores.";
